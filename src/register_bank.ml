...
module Packed_array = struct
  ...
  struct
    ...
    let convert_read_field_to_extract_field read_field (unpacked @ local) =
      read_field (local_ fun ~index -> Array.get unpacked index) [@nontail]
    ;;
    ...
    let extract_field_as_bytes =
      X.map read_field_as_bytes ~f:(fun read (unpacked @ local) b ->
        read (local_ fun ~index -> unpacked.(index)) b [@nontail])
    ;;
    ...
    let of_packed_int_array t =
      X.map extract_field_as_int ~f:(local_ fun extract_fn -> extract_fn t) [@nontail]
    ;;

    let of_packed_int_array_to_int64 t =
      X.map extract_field_as_int64 ~f:(local_ fun extract_fn -> extract_fn t) [@nontail]
    ;;
