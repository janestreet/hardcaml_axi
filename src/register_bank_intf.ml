...
module Pipelined_read_depth = struct
  type t =
    { external_cycles : int
...
struct
  module type S = sig
    ...
    val create
      :  ?pipelined_read_depth:Pipelined_read_depth.t
      -> Scope.t
...
module type S = sig
  ...
  module With_interface (Read : Interface.S) (Write : Interface.S) :
    M_with_interface(Read)(Write)(Master_to_slave)(Slave_to_master).S

  (** Like [With_interface], but with explicit addresses for each register.

      Each address must be a non-negative multiple of 4. Within each direction (read or
      write) addresses must be unique, but they need not be contiguous and need not be
      listed in increasing order. Gaps are filled with placeholder slots in the underlying
      register bank, so very sparse address layouts will instantiate a correspondingly
      large set of (unused) registers. *)
  module With_interface_and_addrs (M : sig
      ...
      val write_addresses : int Write.t
      val read_addresses : int Read.t
    end) : M_with_interface(M.Read)(M.Write)(Master_to_slave)(Slave_to_master).S
end
...
module type Register_bank = sig
  ...
  module Pipelined_read_depth = Pipelined_read_depth
  module M_with_interface = M_with_interface
