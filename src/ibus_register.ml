...
struct
  ...
  let create ~supports_wready scope (i : _ I.t) =
      ...
      ; sm.switch
          [ ( Idle
            , [ Master_to_slave.(Of_always.assign o.master_dn (Of_signal.of_int 0))
              ; if_
                  (i.master_up.write_valid &: i.master_up.write_first)
                  [ (if supports_wready then [ sm.set_next Write ] else []) |> proc
                  ; slave_up_write_ready <-- vdd
                  ; Master_to_slave.Of_always.assign
                      o.master_dn
                      { i.master_up with read_valid = gnd; read_first = gnd }
                  ]
                @@ elif
                     (i.master_up.read_valid &: i.master_up.read_first)
                     [ sm.set_next Read_dn
                     ; Master_to_slave.Of_always.assign
                         o.master_dn
                         { i.master_up with write_valid = gnd; write_first = gnd }
                     ]
                @@ []
              ] )
          ; ( Write
  ...
  let hierarchical ?instance ~supports_wready scope i =
    let module Scoped = Hierarchy.In_scope (I) (O) in
    Scoped.hierarchical ?instance ~scope ~name:"ibus_register" (create ~supports_wready) i
  ;;
end
